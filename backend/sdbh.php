...
class sdbh {
    ...
    public function calculate_rent_cost($product_id, $days, $selected_services) {

        $product = $this->mselect_rows('a25_products', ['ID' => $product_id], 0, 1, 'id')[0];

        $price_per_day = $product['PRICE'];
        $tariffs = unserialize($product['TARIFF']);

        if (!empty($tariffs)) {
            foreach ($tariffs as $tariff_days => $tariff_price) {
                if ($days >= $tariff_days) {
                    $price_per_day = $tariff_price;
                } else {
                    break;
                }
            }
        ...
        $services = unserialize($this->mselect_rows('a25_settings', ['set_key' => 'services'], 0, 1, 'id')[0]['set_value']);
        foreach ($selected_services as $service_id) {
            if (isset($services[$service_id])) {
                $total_cost += $services[$service_id] * $days;
            }
        }
